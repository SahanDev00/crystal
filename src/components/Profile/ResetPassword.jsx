...
const ResetPassword = () => {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        const response = await fetch(`https://kmatadmin.worldpos.biz/api/Token/${tokenID}`,{
            method: 'GET',
            headers: {
              'APIKey': apiKey,
            },
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
    const apiKey = process.env.REACT_APP_API_KEY;
      const response = await fetch(`https://kmatadmin.worldpos.biz/api/Customer/EditPassword?CustomerID=${customerID}&LoginPassword=${newPassword}`, {
        method: 'PUT',
        headers: {
